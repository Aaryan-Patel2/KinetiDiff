...
# Handle torch_scatter import - it may not be available due to CUDA version mismatches
try:
    import torch_scatter
    # Ensure scatter function exists (some versions may not have it)
    if not hasattr(torch_scatter, 'scatter'):
        from torch_scatter_impl import scatter
        torch_scatter.scatter = scatter
except ImportError:
    # Use proper fallback implementation
    import types

    from torch_scatter_impl import scatter, scatter_add, scatter_mean
...
def get_activations(act_name: ActivationType, return_functional: bool = False) -> nn.Module | Callable:
...
@jaxtyped
@beartype
def is_identity(nonlinearity: Callable | nn.Module | None = None) -> bool:
