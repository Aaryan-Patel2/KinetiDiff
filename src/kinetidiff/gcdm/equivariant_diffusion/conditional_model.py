...
# Handle torch_scatter import
try:
    from torch_scatter import scatter_add, scatter_mean
except ImportError:
    # Use proper fallback implementation
    from torch_scatter_impl import scatter_add, scatter_mean

import utils
from equivariant_diffusion.en_diffusion import EnVariationalDiffusion

# Optional Vina guidance
try:
    from equivariant_diffusion.vina_gradient_guidance import VinaGradientGuidance
    VINA_GUIDANCE_AVAILABLE = True
except ImportError:
    VINA_GUIDANCE_AVAILABLE = False


class ConditionalDDPM(EnVariationalDiffusion):
    """
    Conditional Diffusion Module with optional Vina gradient guidance.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert not self.dynamics.update_pocket_coords
        
        # Vina guidance attributes
        self.vina_guidance = None
        self.vina_guidance_scale = 1.0
        self._guidance_enabled = False
    
    def enable_vina_guidance(
        self,
        receptor_pdbqt: str,
        binding_site_config: dict,
        guidance_scale: float = 1.0,
        guidance_start_timestep: int = 400,
        guidance_interval: int = 20,
        n_gradient_atoms: int = 5
    ):
        """
        Enable Vina gradient guidance for sampling.
        
        Call this BEFORE generate_ligands() to enable guided generation.
        
        Args:
            receptor_pdbqt: Path to prepared receptor PDBQT
            binding_site_config: Dict with center_x/y/z and size_x/y/z
            guidance_scale: Base scale for gradients (default 1.0)
            guidance_start_timestep: Only guide when t < this (0-1000)
            guidance_interval: Apply gradient every N steps
            n_gradient_atoms: Atoms to sample for gradient computation
        """
        if not VINA_GUIDANCE_AVAILABLE:
            print("⚠ Vina guidance not available (import failed)")
            return
        
        self.vina_guidance = VinaGradientGuidance(
            receptor_pdbqt=receptor_pdbqt,
            binding_site_config=binding_site_config,
            guidance_start_timestep=guidance_start_timestep,
            guidance_interval=guidance_interval,
            n_gradient_atoms=n_gradient_atoms
        )
        self.vina_guidance_scale = guidance_scale
        self._guidance_enabled = True
        
        print(f"✓ Vina guidance enabled (scale={guidance_scale})")
    
    def disable_vina_guidance(self):
        """Disable Vina guidance."""
        self._guidance_enabled = False
        print("✓ Vina guidance disabled")
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
        ...
        # Pocket center for guidance
        pocket_center = scatter_mean(pocket['x'], pocket['mask'], dim=0).mean(dim=0)
        print(f"  [DEBUG] pocket_center: {pocket_center}")
        ...
        for s in reversed(range(0, timesteps)):
            ...
            # === VINA GRADIENT GUIDANCE ===
            if self._guidance_enabled and self.vina_guidance is not None:
                if self.vina_guidance.should_apply(s, timesteps):
                    # Extract coordinates (first n_dims dimensions) - these are NORMALIZED around zero
                    coords_normalized = z_lig[:, :self.n_dims]
                    atom_types_continuous = z_lig[:, self.n_dims:]
                    
                    # lig_mask is a batch index tensor: [0,0,0,..,1,1,1,..] 
                    # NOT a boolean mask! We need to process each sample separately.
                    
                    # Get binding site center
                    binding_center = torch.tensor([
                        self.vina_guidance.binding_site['center_x'],
                        self.vina_guidance.binding_site['center_y'], 
                        self.vina_guidance.binding_site['center_z']
                    ], device=coords_normalized.device)
                    
                    # Initialize gradient tensor (will accumulate from all samples)
                    gradient_full = torch.zeros_like(coords_normalized)
                    
                    # Process each sample in the batch separately
                    n_samples_in_batch = int(lig_mask.max().item()) + 1
                    
                    for sample_idx in range(n_samples_in_batch):
                        # Get mask for this sample
                        sample_mask = (lig_mask == sample_idx)
                        n_atoms = sample_mask.sum().item()
                        
                        if n_atoms < 3:
                            continue
                        
                        # Extract this sample's coordinates and atom types
                        sample_coords = coords_normalized[sample_mask]
                        sample_atom_types = atom_types_continuous[sample_mask]
                        
                        # Convert to absolute coordinates
                        sample_coords_abs = sample_coords + binding_center.unsqueeze(0)
                        
                        # Convert continuous atom features to discrete indices
                        sample_types_discrete = sample_atom_types.argmax(dim=-1)
                        
                        # Debug at key timesteps (only for first sample)
                        if (s <= 10 or s % 50 == 0) and sample_idx == 0:
                            print(f"  DEBUG t={s}: sample {sample_idx}, n_atoms={n_atoms}")
                            print(f"  DEBUG t={s}: coords_NORMALIZED range: min={sample_coords.min():.2f}, max={sample_coords.max():.2f}")
                            print(f"  DEBUG t={s}: coords_ABSOLUTE range: min={sample_coords_abs.min():.2f}, max={sample_coords_abs.max():.2f}")
                        
                        # Compute Vina gradient for this sample
                        sample_grad, metrics = self.vina_guidance.compute_gradient(
                            sample_coords_abs,
                            sample_types_discrete,
                            torch.ones(n_atoms, device=sample_coords.device),  # All 1s mask for single mol
                            s,
                            binding_center
                        )
                        ...
                        if metrics['valid_molecule'] and metrics['vina_score'] is not None:
                            gradient_full[sample_mask] = sample_grad.to(coords_normalized.device)
                            
                            if s % 5 == 0 and sample_idx == 0:
                                print(f"  t={s}: sample {sample_idx} Vina={metrics['vina_score']:.2f}, "
                                      f"grad_norm={metrics['gradient_norm']:.4f}")
                    
                    # Apply accumulated gradient (SUBTRACT to MINIMIZE Vina score)
                    if gradient_full.abs().sum() > 0:
                        scale = self.vina_guidance.get_adaptive_scale(s, self.vina_guidance_scale)
                        # CRITICAL: Subtract gradient to minimize Vina (gradient points toward INCREASING score)
                        coords_guided = coords_normalized - scale * gradient_full
                        
                        # Re-center to maintain zero-mean constraint (per sample)
                        for sample_idx in range(n_samples_in_batch):
                            sample_mask = (lig_mask == sample_idx)
                            sample_coords = coords_guided[sample_mask]
                            sample_mean = sample_coords.mean(dim=0)
                            coords_guided[sample_mask] = sample_coords - sample_mean
                        
                        z_lig = torch.cat([coords_guided, atom_types_continuous], dim=1)
...
class SimpleConditionalDDPM(ConditionalDDPM):
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
        ...
        return super().sample_given_pocket(
