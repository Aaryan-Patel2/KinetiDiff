...
try:
    ...
    # Create dummy module if torch_scatter not available
    import types
...
from equivariant_diffusion.common import (
    centralize,
    decentralize,
    get_activations,
    is_identity,
...
class GCPDropout(nn.Module):
    ...
    def forward(self, x: torch.Tensor | ScalarVector) -> torch.Tensor | ScalarVector:
        if (isinstance(x, torch.Tensor) and x.shape[0] == 0) or (isinstance(x, ScalarVector) and (x.scalar.shape[0] == 0 or x.vector.shape[0] == 0)):
            return x
...
class GCP(nn.Module):
    def __init__(
            ...
            nonlinearities: tuple[str | None] = ("silu", "silu"),
    ...
    def scalarize(
        ...
        dim_size: int,
        node_mask: Bool[torch.Tensor, "n_nodes"] | None = None
    ...
    def forward(
        self,
        s_maybe_v: tuple[Float[torch.Tensor, "batch_num_entities scalar_dim"], Float[torch.Tensor, "batch_num_entities m vector_dim"]] | Float[torch.Tensor, "batch_num_entities merged_scalar_dim"],
        edge_index: Int64[torch.Tensor, "2 batch_num_edges"],
        frames: Float[torch.Tensor, "batch_num_edges 3 3"],
        node_inputs: bool = False,
        node_mask: Bool[torch.Tensor, "batch_num_nodes"] | None = None
    ) -> tuple[Float[torch.Tensor, "batch_num_entities new_scalar_dim"], Float[torch.Tensor, "batch_num_entities n vector_dim"]] | Float[torch.Tensor, "batch_num_entities new_scalar_dim"]:
        if self.vector_input_dim:
...
class GCPEmbedding(nn.Module):
    ...
    def forward(
        ...
        tuple[Float[torch.Tensor, "batch_num_nodes h_hidden_dim"], Float[torch.Tensor, "batch_num_nodes m chi_hidden_dim"]] | Float[torch.Tensor, "batch_num_nodes h_hidden_dim"],
        tuple[Float[torch.Tensor, "batch_num_edges e_hidden_dim"], Float[torch.Tensor, "batch_num_edges x xi_hidden_dim"]] | Float[torch.Tensor, "batch_num_edges e_hidden_dim"]
    ]:
...
class GCPInteractions(nn.Module):
    def __init__(
        ...
        dropout: float = 0.0,
        nonlinearities: tuple[Any, Any] | None = None
    ):
        super().__init__()
    ...
    def derive_x_update(
        ...
        node_mask: Bool[torch.Tensor, "batch_num_nodes"] | None = None,
        fixed_pos_mask: Bool[torch.Tensor, "batch_num_nodes"] | None = None,
        fix_pos: bool = False
    ...
    def forward(
        ...
        node_mask: Bool[torch.Tensor, "batch_num_nodes"] | None = None,
        fixed_pos_mask: Bool[torch.Tensor, "batch_num_nodes"] | None = None,
        node_pos: Float[torch.Tensor, "batch_num_nodes 3"] | None = None,
        fix_pos: bool = False
    ) -> tuple[
            tuple[
            ...
            Float[torch.Tensor, "batch_num_nodes 3"] | None
        ]:
